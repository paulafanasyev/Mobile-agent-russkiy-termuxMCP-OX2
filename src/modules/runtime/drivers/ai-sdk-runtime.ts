...
function sanitizeToolSet(tools: GenerateModelTextStreamParams["tools"]) {
...
function mergeDeviceTools(
  ...
  if (Platform.OS !== "android") {
    return sanitizeToolSet(runtimeTools);
  }

  const deviceTools = createDeviceToolSet();
  return sanitizeToolSet({
    ...deviceTools,
    ...(runtimeTools ?? {}),
  ...
  try {
    ...
    try {
      ...
      if (!finalText.trim() && toolResults.length > 0 && emptyToolContinuationCount < 2 && steps.length < params.maxToolSteps) {
        return generateViaAISDKWithContinuation(providerModel, {
          ...
          messages: [
            ...params.messages,
            ...responseMessages,
            { role: "user", content: "Continue the original task using the tool results above. Keep calling tools until the requested work is complete, then provide a final response." },
          ],
      ...
      return {
        ...
        toolResults,
        usage,
        stepLimitReached: steps.length >= params.maxToolSteps && steps.at(-1)?.finishReason === "tool-calls",
      };
    } catch (error) {
...
export async function generateViaAISDKNonStreaming(
  ...
  return {
    ...
    usage: result.usage,
    stepLimitReached: result.steps.length >= params.maxToolSteps && result.steps.at(-1)?.finishReason === "tool-calls",
  };
}
