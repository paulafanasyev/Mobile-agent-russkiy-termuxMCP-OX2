  ...
  override fun definition() = ModuleDefinition {
    ...
    AsyncFunction("captureCamera") { promise: Promise ->
      val context = appContext.reactContext ?: run {
        promise.resolve(mapOf("status" to "camera_failed", "verified" to false, "reason" to "Android context unavailable"))
        return@AsyncFunction
      }
      if (Build.VERSION.SDK_INT < 29) {
        promise.resolve(mapOf("status" to "unsupported_android_version", "verified" to false, "minimumApi" to 29))
        return@AsyncFunction
      }
      if (pendingCameraPromise != null) {
        promise.resolve(mapOf("status" to "camera_busy", "verified" to false))
        return@AsyncFunction
      }
      ...
      val uri = context.contentResolver.insert(MediaStore.Images.Media.EXTERNAL_CONTENT_URI, values) ?: run {
        promise.resolve(mapOf("status" to "camera_failed", "verified" to false, "reason" to "MediaStore insert failed"))
        return@AsyncFunction
      }

      val launcher = cameraLauncher ?: run {
        context.contentResolver.delete(uri, null, null)
        promise.resolve(mapOf("status" to "camera_failed", "verified" to false, "reason" to "Camera launcher unavailable"))
        return@AsyncFunction
      }
      ...
      try {
        launcher.launch(
          ...
          ActivityResultCallback { result -> resolveCameraResult(result) }
        )
      } catch (t: Throwable) {
